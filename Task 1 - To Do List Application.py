...
class todolist:
    def __init__(self,main):
        ...
        close_btn=ttk.Button(self.main,text="Close",command=self.close).grid(row=5,column=0,pady=10,padx=10)
    def view(self):
        view_win=tk.Tk()
    ...
    def close(self):
        try:
            if self.window==self.view_win2:
                self.window.destroy()
                self.window=self.view_win
            else:
                self.window.destroy()
                self.window=self.main
        except:
            if self.window!=self.main:
                self.window.destroy()
                self.window=self.main
            else:
                self.main.destroy()
    def check1(self):
        ...
        if checking ==True:
            if self.mode=="view":
                ...
                ttk.Label(view_main2,text=m.upper(),font=("calibri",18)).grid(row=0,column=0,sticky=tk.N,padx=30,pady=10)
                c=1
                for i in lines:
